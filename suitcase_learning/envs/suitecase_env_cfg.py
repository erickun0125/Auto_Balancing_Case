...
def get_environment_groups(num_envs: int, num_groups: int = 4):
    """Partition environments into non-overlapping groups.
    ...
    Returns:
        List of tensors, each containing the environment indices for one group.
    """
    ...
    for i in range(num_groups):
        # Assign remaining environments to the last group
        if i == num_groups - 1:
            end_idx = num_envs
        else:
...
def configure_event_groups(env_cfg, num_envs: int):
    """Assign each disturbance event to a separate environment partition.
    ...
    # Apply each event to its dedicated group (convert Tensor to list)
...
# USD asset path (relative to this file)
import os as _os
DEFAULT_SUITECASE_USD = _os.path.join(_os.path.dirname(__file__), "..", "assets", "Auto_Balancing_Case.usda")
...
class RewardsCfg:
    ...
    # (0) Survival reward — incentivize staying upright
    is_alive = RewTerm(
        func=mdp.is_alive,
        weight=1.0,
    )

    # (1) Termination penalty — large punishment on episode failure
    is_terminated = RewTerm(
        func=mdp.is_terminated,
        weight=-100.0,
    )

    # (2) Keep base platform horizontal
    ...
    # (3) Minimize angular velocity (reduces oscillation)
    ...
    # (4) Stay near default hinge position
    ...
    # (5) Maintain all 4 wheel contacts with ground
    ...
    # (6) Equal force distribution across wheels (variance-based, exp(-var/mean))
    ...
    # (7) Penalize max-min contact force difference
